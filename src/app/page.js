...
export default function DarkApp(){
  ...
  const fetchChannelIdFromUrl=async(url,setFn)=>{
    const apiKey=process.env.NEXT_PUBLIC_YOUTUBE_API_KEY;
    if(!apiKey||!url)return;
    let handle=null;
    const matchHandle=url.match(/@([^/?&]+)/);
    const matchChannel=url.match(/channel\/([^/?&]+)/);
    if(matchChannel){setFn(prev=>({...prev,channel_id:matchChannel[1]}));return;}
    if(matchHandle)handle=matchHandle[1];
    else handle=url.replace(/https?:\/\/(www\.)?youtube\.com\//,"").replace(/\//,"");
    if(!handle)return;
    try{
  ...
  return (
    <div style={{background:BG,minHeight:"100vh",color:TEXT}}>
      ...
      {trendingRefModal&&trendingRefEdit&&<div onClick={e=>e.target===e.currentTarget&&(setTrendingRefModal(false),setTrendingRefEdit(null))} style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.85)",zIndex:100,display:"flex",alignItems:"center",justifyContent:"center",padding:20}}><div style={{background:BG2,border:"1px solid "+BOR2,borderRadius:12,width:"100%",maxWidth:460,padding:26}}><div style={{display:"flex",justifyContent:"space-between",marginBottom:18}}><div style={{fontFamily:"'Bebas Neue'",fontSize:18,letterSpacing:2}}>{trendingRefEdit.id?"EDITAR CANAL":"NOVO CANAL REFERÊNCIA"}</div><button onClick={()=>{setTrendingRefModal(false);setTrendingRefEdit(null);}} style={btnGhost}>✕</button></div><div style={{marginBottom:12}}><span style={lbl}>Nome do canal</span><input value={trendingRefEdit.name||""} onChange={e=>setTrendingRefEdit({...trendingRefEdit,name:e.target.value})} placeholder="Ex: Cansei de ser Ot4rio" style={inp}/></div><div style={{marginBottom:12}}><span style={lbl}>URL do canal</span><div style={{display:"flex",gap:6}}><input value={trendingRefEdit.url||""} onChange={e=>setTrendingRefEdit({...trendingRefEdit,url:e.target.value})} placeholder="https://youtube.com/@canal" style={{...inp,flex:1}}/><button onClick={()=>fetchChannelIdFromUrl(trendingRefEdit.url,setTrendingRefEdit)} style={{...btnGold,whiteSpace:"nowrap",fontSize:12,padding:"8px 12px"}}>🔍 Buscar ID</button></div>{trendingRefEdit.channel_id&&<div style={{fontFamily:"'DM Sans'",fontSize:11,color:GREEN,marginTop:5}}>✓ ID encontrado: {trendingRefEdit.channel_id}</div>}</div><div style={{marginBottom:14}}><span style={lbl}>Notas</span><textarea value={trendingRefEdit.notes||""} onChange={e=>setTrendingRefEdit({...trendingRefEdit,notes:e.target.value})} placeholder="Por que esse canal é referência..." style={{...inp,minHeight:55}}/></div><div style={{display:"flex",gap:9,justifyContent:"flex-end"}}><button onClick={()=>{setTrendingRefModal(false);setTrendingRefEdit(null);}} style={btnGhost}>Cancelar</button><button onClick={saveTrendingRefChannel} style={btnGold}>SALVAR</button></div></div></div>}
